<script lang="ts">
  ...
  function SettingChange(settingData: StaticData_Setting, optionData: StaticData_SettingOption) {
    ...
    saveDataMainStoreReducers.settingChange(activitySaveData.activityId, settingData.settingName, optionData.settingOptionName);
...
$:borderOnHover=()=>{
  if(hovered)  return "border-4 border-cyan-600"
  return ""
}
</script>

<span on:mouseenter on:mouseleave class="mrkCard {borderOnHover()}" style="  min-width: 10rem;"

>

  <div >
        <h3 class="justify-center text-2xl text-center"> {activityType}</h3>
    {#if (activityCardState == ActivityCardStates.Picker)}
            <div class="bg-green-600 w-7 flex justify-center rounded-full hover:bg-green-700 float-right"
                 on:click={()=>{saveDataMainStoreReducers.addActivity(activityType)}}
            >

                <i style="float: right" class="mi-add" />
            </div>
  ...
  {#each ActivityStaticData.activitySettings as setting, i}
        {#if (setting.settingType === SettingsTypes.boolean)}
          <SettingBool settingStaticData={setting}></SettingBool>
        {:else if (setting.settingType === SettingsTypes.dropDown)}
            <SettingDropDown settingStaticData={setting}
                             on:SettingChange={(e)=>{SettingChange(setting,e.detail)}}
                             settingSaveData={settingSaveFunc(setting.settingName)}></SettingDropDown>
        {/if}
