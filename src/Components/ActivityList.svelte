...
<div>
  <div style="background-color:rgba(143,143,143,0.16);margin:10px;border:15px;padding:5px;border-radius: 20px">
      ...
      <div class=" w-full overflow-auto flex flex-col  " style="white-space:nowrap;">
        <div class=" w-full  flex mb-2 " style="white-space:nowrap;">
          {#each $saveDataMainStore.activityList as data, i}
            ...
            <ActivityTimeElement on:mouseenter={mouseEnterFunc(data.activityId)} on:mouseleave={mouseEnterFunc()}
        ...
        <div class=" w-full  flex" style="white-space:nowrap;">
          ...
          {#each $saveDataMainStore.activityList as data, i}
            ...
            <ActivityCard on:mouseenter={mouseEnterFunc(data.activityId)} on:mouseleave={mouseEnterFunc()}
                          hovered={activityHoveredFunc(data.activityId)} activityType={data.activityType}
                          activitySaveData={data}
                          activityCardState={ActivityCardStates.List}></ActivityCard>
